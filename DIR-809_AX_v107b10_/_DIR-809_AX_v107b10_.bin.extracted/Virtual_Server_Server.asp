...
return ("" !== "") ? "/Basic/Setup.asp" : "/Advanced/Virtual_Server_Server.asp";
}
...
if (mf["vs_enabled_" + i].value == "true" || mf["vs_enabled_" + i].value == true ||vs_name !=""|| vs_private !="" || vs_public !="" ||vs_ipaddr !="" ) {
...
if (vs_name == "") 
{
	if (vs_ipaddr !="" || vs_public !="" || vs_private !="")
	{
...
alert(sw("txtVirServer") +" '"+vs_name+"'"+sw("txtDe")+ sw("txtIPaddressInvalid") + "!");//modified by gold
...
//	chk_end=parseInt(mf["public_port_to_" + k].value, [10]);
	...
	//if((start<=chk_start && end>=chk_start)||(start>=chk_start))
    if((start == chk_start))
	{
		//alert(sw("txtInvalidPort"));
...
for(var k = 0; k < MAXNUM_VIRTUAL_SERVER; k++)
{
    ...
    if((chk_start <= 0) || (chk_end <= 0))
        continue;

    if((start >= chk_start) && (start <= chk_end))
    {
...
for(var k = 0; k < MAXNUM_VIRTUAL_SERVER; k++)
{
    ...
    for (var j = 0; j < pubport.length; j++)
    {
        ...
        {
            if ((start - ports[0] >= 0) && (ports[1] - start >= 0))
            {
                alert(sw("txtPublic")+" "+sw("txtConflictWithStr2"));
                mf["vs_public_" + i].select();
                mf["vs_public_" + i].focus();
...
if ((priv_port > 65535) ||(priv_port < 0))
{
	alert(sw("txtInvalidPort"));
	mf["vs_private_" + i].select();
	mf["vs_private_" + i].focus();	
...
if( allow_wan_http == "true") {
	if (vs_proto == "1" || vs_proto == "3") {
		...
		alert(sw("txtVirtualAppStr1") + "'" + vs_name + "'" );
		return 0;
		}
	}
}
...
{
alert(sw("txtVirServer")+" '"+mf["vs_name_" + i].value+"'"+sw("txtDeIPAddr")+sw("txtShouldWithinLanSubnet"));
return false;
}
}
}
...
function page_submit()
{
    ...
    var auth = hex_hmac_md5(PrivateKey, current_time.toString()+"/Advanced/Virtual_Server_Server.asp");
...
{
token[0]="<% virtualAppList("virtualApp_1");%>"
token[1]="<% virtualAppList("virtualApp_2");%>"
token[2]="<% virtualAppList("virtualApp_3");%>"
token[3]="<% virtualAppList("virtualApp_4");%>"
token[4]="<% virtualAppList("virtualApp_5");%>"
token[5]="<% virtualAppList("virtualApp_6");%>"
token[6]="<% virtualAppList("virtualApp_7");%>"
token[7]="<% virtualAppList("virtualApp_8");%>"
token[8]="<% virtualAppList("virtualApp_9");%>"
token[9]="<% virtualAppList("virtualApp_10");%>"
token[10]="<% virtualAppList("virtualApp_11");%>"
token[11]="<% virtualAppList("virtualApp_12");%>"
token[12]="<% virtualAppList("virtualApp_13");%>"
token[13]="<% virtualAppList("virtualApp_14");%>"
token[14]="<% virtualAppList("virtualApp_15");%>"
token[15]="<% virtualAppList("virtualApp_16");%>"
token[16]="<% virtualAppList("virtualApp_17");%>"
token[17]="<% virtualAppList("virtualApp_18");%>"
token[18]="<% virtualAppList("virtualApp_19");%>"
token[19]="<% virtualAppList("virtualApp_20");%>"
token[20]="<% virtualAppList("virtualApp_21");%>"
token[21]="<% virtualAppList("virtualApp_22");%>"
token[22]="<% virtualAppList("virtualApp_23");%>"
token[23]="<% virtualAppList("virtualApp_24");%>"
...
document.write("		<input type=\"text\" size=\"4\" maxlength=\"5\" id=\"vs_public_"+i+"\" name=\"public_port_"+i+"\" value=\""+DataArray[3]+"\" />");
//document.write("		&nbsp;~&nbsp;<input type=\"text\" size=\"4\" maxlength=\"5\" id=\"public_port_to_"+i+"\" name=\"public_port_to_"+i+"\" value=\""+DataArray[8]+"\"  />");
document.write("	</td>");
document.write("	<td class=\"centered\" rowspan='2'>");
document.write("		<br />");
...
document.write("		<input type=\"text\" size=\"4\" maxlength=\"5\" id=\"vs_private_"+i+"\" name=\"private_port_"+i+"\" value=\""+DataArray[4]+"\" />");
...
<form name="mainform" action="/formVirtualApp.htm" method="post" enctype="application/x-www-form-urlencoded" id="mainform">
...
<input type="hidden" value="/Advanced/Virtual_Server_Server.asp" name="submit-url">
<div class="section">
<div class="section_head">
<h2><SCRIPT >ddw("txtVirServer");</SCRIPT></h2>
<p><SCRIPT >ddw("txtVirtualAppStr");</SCRIPT></p>
<SCRIPT language=javascript>DrawSaveButton();</SCRIPT>
</div>
<div class="box">
<h3>24--<SCRIPT >ddw("txtVirtualApp");</SCRIPT></h3>
...
<SCRIPT >ddw("txtRemainRulesCanbeCreated");</SCRIPT>
 : <font color=red>
<%getIndexInfo("reamin_virtual_app_num");%> 	
</font>
<br><br>
