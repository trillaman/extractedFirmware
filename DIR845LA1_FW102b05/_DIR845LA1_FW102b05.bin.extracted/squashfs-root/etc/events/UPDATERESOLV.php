...
//Care about the Super-DMZ function
$sdmz_src_lanip = "";
if(get("", "/nat/entry/sdmz/enable")=="1")	
{$sdmz_src_lanip = "src ".INF_getcurripaddr($LAN1);} 
			...
			foreach ("inet/".$addrtype."/dns")
			{
				...
				if ($def!="" && $def>1)
				{
					if ($gw!="")
					{
						...
						fwrite(a,$START,'ip route add '.$VaLuE.' via '.$gw.' metric '.$def.' table RESOLV '.$sdmz_src_lanip.'\n');
						fwrite(a,$STOP, 'ip route del '.$VaLuE.' via '.$gw.' metric '.$def.' table RESOLV '.$sdmz_src_lanip.'\n');
					}
					else
					{
						fwrite(a,$START,'ip route add '.$VaLuE.' metric '.$def.' table RESOLV '.$sdmz_src_lanip.'\n');
						fwrite(a,$STOP, 'ip route del '.$VaLuE.' metric '.$def.' table RESOLV '.$sdmz_src_lanip.'\n');
					}
				}
				else
				{
					if ($upperlayer!="" && $gw!="")
					{
						...
						*/
						...
						fwrite(a,$START,'ip route add '.$VaLuE.' via '.$gw.' metric '.$ppp_metric.' table RESOLV '.$sdmz_src_lanip.'\n');
						fwrite(a,$STOP, 'ip route del '.$VaLuE.' via '.$gw.' metric '.$ppp_metric.' table RESOLV '.$sdmz_src_lanip.'\n');
					}	
					else if ($gw!="")
					{
						fwrite(a,$START,'ip route add '.$VaLuE.' via '.$gw.' table RESOLV '.$sdmz_src_lanip.'\n');
						fwrite(a,$STOP, 'ip route del '.$VaLuE.' via '.$gw.' table RESOLV '.$sdmz_src_lanip.'\n');
					}
					else
					{
						fwrite(a,$START,'ip route add '.$VaLuE.' table RESOLV '.$sdmz_src_lanip.'\n');
						fwrite(a,$STOP, 'ip route del '.$VaLuE.' table RESOLV '.$sdmz_src_lanip.'\n');
					}
				}
			}
		}
...
if ($opendns_type == "family" || $opendns_type == "parent")
{
	...
	fwrite(a,$START,'ip route add '.query($wan1_infp."/open_dns/".$opendns_type."_dns_srv/dns1").' table RESOLV '.$sdmz_src_lanip.'\n');
	fwrite(a,$START,'ip route add '.query($wan1_infp."/open_dns/".$opendns_type."_dns_srv/dns2").' table RESOLV '.$sdmz_src_lanip.'\n');
	fwrite(a,$STOP,'ip route del '.query($wan1_infp."/open_dns/".$opendns_type."_dns_srv/dns1").' table RESOLV '.$sdmz_src_lanip.'\n');
	fwrite(a,$STOP,'ip route del '.query($wan1_infp."/open_dns/".$opendns_type."_dns_srv/dns2").' table RESOLV '.$sdmz_src_lanip.'\n');
}
