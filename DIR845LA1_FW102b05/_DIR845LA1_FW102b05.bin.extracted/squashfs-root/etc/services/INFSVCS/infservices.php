...
function infsvcs_wan($index)
{
	...
	if ($upper=="")
	{
		...
		/* For a bug issue that if we sync local computer time without enableing NTP, then we will 
		   use the click time we saved previously. This will cause the time to go back. Thereby, we
...
//in webUI the information will be wrong.
	...
	/*It should restart the DMZ service when the WAN connection is completed.*/
	if($index==1 && get("", "/nat/entry/sdmz/enable")=="1")
	{
		startcmd("service DMZ.NAT-1 restart\n");
	}
	/*It should restart the DEVICE service when the WAN connection is completed for page redirect, sammy */
	if($index==1 && isfile("/etc/services/DEVICE.php")==1)
	{
		startcmd("service DEVICE restart\n");
	}
