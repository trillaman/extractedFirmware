...
function dhcps4start($inf, $phyinf, $dhcpsp)
{
	...
	//+++ Jerry Kao, Add Function: Super DMZ for KR in DIR-845.
	$sdmz_enable = query("/nat/entry/sdmz/enable");		
	
	if ($sdmz_enable == "1")
	{						
		$WAN1 = "WAN-1";
		$sdmz_mac  = query("/nat/entry/sdmz/mac");
		//Transfer 00:22:33:55:E6:77 to 00223355e677
		$sdmz_mac = tolower($sdmz_mac);
		$sdmz_mac = substr($sdmz_mac, 0, 2).substr($sdmz_mac, 3, 2).substr($sdmz_mac, 6, 2).substr($sdmz_mac, 9, 2).substr($sdmz_mac, 12, 2).substr($sdmz_mac, 15, 2);
		$sdmz_stsp = XNODE_getpathbytarget("/runtime", "inf", "uid", $WAN1, 0);
		$wanaddr_type = INF_getcurraddrtype($WAN1);
		
		if($wanaddr_type=="ipv4")
		{
			$sdmz_ip	 = query($sdmz_stsp."/inet/ipv4/ipaddr");		
			$sdmz_router = query($sdmz_stsp."/inet/ipv4/gateway");
			$sdmz_subnet = ipv4int2mask(query($sdmz_stsp."/inet/ipv4/mask"));
			$sdmz_dns_1  = query($sdmz_stsp."/inet/ipv4/dns:1");
			$sdmz_dns_2  = query($sdmz_stsp."/inet/ipv4/dns:2");				
		}
		else if($wanaddr_type=="ppp4")
		{
			$sdmz_ip	 = query($sdmz_stsp."/inet/ppp4/local");		
			$sdmz_router = query($sdmz_stsp."/inet/ppp4/peer");
			$sdmz_subnet = ipv4int2mask(query($sdmz_stsp."/inet/ppp4/mask"));
			$sdmz_dns_1  = query($sdmz_stsp."/inet/ppp4/dns:1");
			$sdmz_dns_2  = query($sdmz_stsp."/inet/ppp4/dns:2");			
		}
		
		if($sdmz_ip=="") //If WAN IP is empty, udhcpd /var/servd/LAN-1-udhcpd.conf would be failed.
		{
			$sdmz_ip	= "192.168.254.253";
			$sdmz_subnet= "255.255.255.0";
			$sdmz_router= "";
			$sdmz_lease= "20";
		}
		else
		{
			/*if we got ip ,let sdmz pc have longer lease time*/
			$sdmz_lease= "300";
		}
		fwrite("a",$udhcpd_conf, "sdmz yes\n");
		fwrite("a",$udhcpd_conf, "static Test-PC ". $sdmz_ip ." ". $sdmz_mac ."\n");		
		fwrite("a",$udhcpd_conf, "sdmzmac ". $sdmz_mac ."\n");	
		if($sdmz_router!="")
		{fwrite("a",$udhcpd_conf, "sdmzopt router ". $sdmz_router. "\n");}
		fwrite("a",$udhcpd_conf, "sdmzopt subnet ". $sdmz_subnet ."\n");
		fwrite("a",$udhcpd_conf, "sdmzopt lease ". $sdmz_lease ."\n");
		if($sdmz_dns_1!="")
		{fwrite("a",$udhcpd_conf, "sdmzopt dns ". $sdmz_dns_1. "\n");}
		if($sdmz_dns_2!="")
		{fwrite("a",$udhcpd_conf, "sdmzopt dns ". $sdmz_dns_2. "\n");}	
	}
