...
function check_ppp4($path)
{
	...
	if ($mtu != "")
	{
		...
		{
			set_result("FAILED",$path."/mtu",
				i18n("The MTU value is too large, the valid value for pptp is 576 ~ 1460."));
			return;
		}
		else if ($over=="l2tp" && $mtu > 1460)
		{
			set_result("FAILED",$path."/mtu",
				i18n("The MTU value is too large, the valid value for l2tp is 576 ~ 1460."));
			return;
		}
		else if ($mtu > 1492)
		{
	...
	if ($tout != "")
	{
		if (isdigit($tout)=="0" || $tout < 0 || $tout >= 10000)
		{
			set_result("FAILED",$path."/dialup/mode",
				i18n("Invalid value for idle timeout, the valid value is 0 ~ 9999."));
			return;
		}
	}
