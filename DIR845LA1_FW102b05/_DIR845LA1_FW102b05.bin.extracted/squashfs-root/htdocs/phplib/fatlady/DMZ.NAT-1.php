...
function check_sdmz_setting($path)
{
	anchor($path);
	$enable = query("enable");
	if($enable=="1")
	{
		...
		if ($mac == "")
		{
			set_result("FAILED", $entry."/mac", i18n("No MAC address value."));
			return "FAILED";
		}
		if (PHYINF_validmacaddr($mac) != 1)
		{
			set_result("FAILED", $entry."/mac", i18n("Invalid MAC address value."));
			return "FAILED";
		}
	}
	return "OK";	
...
set_result("FAILED","","");
if (check_dmz_setting($FATLADY_prefix."/nat/entry:".$InDeX."/dmz", $addrtype, $lan_ip, $mask)=="OK" && check_sdmz_setting($FATLADY_prefix."/nat/entry/sdmz")=="OK")
{
