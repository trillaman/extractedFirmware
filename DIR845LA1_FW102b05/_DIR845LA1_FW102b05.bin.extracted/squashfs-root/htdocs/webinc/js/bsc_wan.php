...
Page.prototype =
{
	...
	services: "DEVICE.LAYOUT,DEVICE.HOSTNAME,PHYINF.WAN-1,INET.BRIDGE-1,INET.INF,WAN,RUNTIME.INF.WAN-3,INET.LAN-4,RUNTIME.INF.WAN-1,RUNTIME.INF.WAN-4,DHCPC4.WAN,REBOOT",
	OnLoad: function() {},
	OnUnload: function() {},
	dhcpc4: null,
	OnSubmitCallback: function (code, result)
	{
		...
		case "OK":
			...
			else if(COMM_Equal(OBJ("ipv4_macaddr").getAttribute("modified"), true) || COMM_Equal(OBJ("ppp4_macaddr").getAttribute("modified"), true))
			{
				var msgArray = ['<?echo i18n("It would spend a little time, please wait");?>...'];	
				BODY.ShowCountdown('<?echo i18n("Clone MAC Address");?>...', msgArray, this.bootuptime, "http://<?echo $_SERVER['HTTP_HOST'];?>/bsc_wan.php");
			}	
			else
			{
				BODY.OnReload();
			...
			switch (over)
			{
				case "eth":
					...
					*/
					...
					//+++ Jerry Kao, Modified the "Maximum Idle Time" follows ppp4 when "Share with IPv4" in PPPoE v6.
	...
	PreSubmit: function()
	{
		...
		{
			//XS(this.wan3.infp+"/inet","INET-8");
			//XS(this.wan3.infp+"/infnext","");
			//XS(this.wan4.infp+"/active","0");
			BODY.ShowAlert("<?echo i18n("IPv6 PPPoE is share with IPv4 PPPoE. Please change IPv6 WAN protocol to PPPoE at first.");?>");
			return null;
		}
	...
	InitPpp4Value: function()
	{
		...
		OBJ("pptp_max_idle_time").value		= 5;
		OBJ("pptp_dns1").value				= "";
		OBJ("pptp_dns2").value				= "";
		...
		OBJ("l2tp_max_idle_time").value		= 5;
		OBJ("l2tp_dns1").value				= "";
		OBJ("l2tp_dns2").value				= "";
		...
		case "pptp":
			...
			OBJ("pptp_max_idle_time").value	= XG(this.wan1.inetp+"/ppp4/dialup/idletimeout")==""?5:XG(this.wan1.inetp+"/ppp4/dialup/idletimeout");
		...
		case "l2tp":
			...
			OBJ("l2tp_max_idle_time").value	= XG(this.wan1.inetp+"/ppp4/dialup/idletimeout")==""?5:XG(this.wan1.inetp+"/ppp4/dialup/idletimeout");
			var dnscount = XG(this.wan1.inetp+"/ppp4/dns/count");
	...
	PrePppoe: function()
	{
		...
		{
			BODY.ShowAlert("<?echo i18n("The user name can not be empty");?>");			
			return null;
		}
	...
	PreL2tp: function(type)
	{
		...
		if (OBJ("l2tp_static").checked)
		{
			...
			if (OBJ("l2tp_dns1").value === "")	{ BODY.ShowAlert("<?echo i18n("Invalid Primary DNS address .");?>"); return null; }
	...
	PreDSLite: function()
	{
		...
		else
		{
			...
			if(wanmode6 !="" && wanactive6 !="0") 
			{
				...
				XS(this.wan1.infp+"/infprevious", "WAN-4");
				XS(this.wan4.infp+"/infnext", "WAN-1");
				if(wanmode6=="STATIC")	XS(this.wan4.infp+"/infnext:2", "LAN-4");
		...
		XS(this.wan1.inetp+"/ipv4/mtu",			"1452");
	...
	OnChangeWanIpMode: function()
	{
		...
		OBJ("box_wan_ru_l2tp").style.display	= "none";
		...
		*/
		...
		case "r_l2tp":		
			OBJ("ppp4_setting").style.display				= "block";
			OBJ("box_wan_ru_l2tp").style.display			= "block";			
			OBJ("box_wan_l2tp_body").style.display			= "block";
			OBJ("box_wan_ppp4_comm_body").style.display		= "block";
			if (XG(this.wan1.inetp+"/ppp4/mtu")=="" || 
			   (over!="l2tp" && parseInt(XG(this.wan1.inetp+"/ppp4/mtu"))>1400))		OBJ("ppp4_mtu").value = "1400";
	...
	{
		//+++ Jerry Kao, marked for enable Reconnect Mode="On demand" in IPv4 PPPoE, 
		//               when Reconnect Mode="Always on" in IPv6 PPPoE.
		/*
