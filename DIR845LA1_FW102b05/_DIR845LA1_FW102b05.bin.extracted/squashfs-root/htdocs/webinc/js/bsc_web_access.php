...
Page.prototype =
{
	...
	{
		//The port 80 and LAN interface are bound already. It should not bind the same port and interface for this service.
		if (!TEMP_IsDigit(OBJ("webaccess_httpport").value) || OBJ("webaccess_httpport").value==80)
		{
			BODY.ShowAlert("<?echo i18n("The HTTP Access Port number is not valid.");?>");
			return false;
		}
		if (!TEMP_IsDigit(OBJ("webaccess_httpsport").value) || OBJ("webaccess_httpsport").value==80)
		{
	...
	InsectUserListTable: function()
	{
		...
		for(var i=0; i < this.access_rule.length; i++)
		{
			...
			if(this.access_rule[i].permission==="rw") var permission = "Read/Write";
			else var permission = "Read Only";
