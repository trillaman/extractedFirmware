...
{
	<?
		if(isfile("/etc/services/UPNPC.php")==1) echo 'services: "DEVICE.ACCOUNT,HTTP.WAN-1,HTTP.WAN-2,INBFILTER,SHAREPORT,UPNPC",';
		else echo 'services: "DEVICE.ACCOUNT,HTTP.WAN-1,HTTP.WAN-2,INBFILTER,SHAREPORT",';
	?>	
	OnLoad: function()
	{
		if (!this.rgmode)
	...
	https_rport: null,
	inbfilter: null,
	sdmz: <? if(get("", "/nat/entry/sdmz/enable")=="1"){echo true;} else{echo false;}?>,
	...
	Initial: function()
	{
		...
		//Disable the remote management if Super-DMZ is enabled.
		if(this.sdmz)
		{ 
			OBJ("en_remote").checked = false;
			OBJ("en_remote").disabled = true;
