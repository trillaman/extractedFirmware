...
Page.prototype =
{
	services: "RUNTIME.PHYINF,INET.WAN-1,INET.WAN-2,INET.WAN-3,INET.WAN-4,INET.LAN-4,RUNTIME.INF.WAN-1,RUNTIME.INF.WAN-3,RUNTIME.INF.WAN-4,WAN,DHCPS6.LAN-4",
	OnLoad: function()
	{
	...
	wanTypes: new Array ("PPPoE", "AUTO", "STATIC", "6RD", "DSLITE"),
	...
	InitWANSettings: function()
	{
		...
		{
			tRTPHYphyinf = GPBT(tRTPHYsvcs+"/runtime", "phyinf", "uid", "ETH-3");
	...
	PreWANSettings: function()
	{
		...
		if(this.wanTypes[this.currentWanType]=="DSLITE")
		{
			XS(this.inf1p+"/infprevious", "WAN-4");
			XS(this.inf1p+"/nat", "");
			XS(this.inet1p+"/addrtype", "ipv4");
			XS(this.inet1p+"/ipv4/static", "0");
			XS(this.inet1p+"/ipv4/ipv4in6/mode", "dslite");
			XS(this.inet1p+"/ipv4/mtu", "1452");
			XS(this.inf4p+"/infnext", "WAN-1");
			XS(this.inf4p+"/defaultroute", "1");
			XS(this.inf4p+"/child", "LAN-4");
			XS(this.inet4p+"/ipv6/mode", "AUTO");
			XS(this.inet4p+"/ipv6/dhcpopt", "IA-NA+IA-PD");
	...
	OnClickUsell: function()
	{
		if (OBJ("usell").checked)
		{
			OBJ("wiz_static_wan_v6addr").disabled 	= true;
			OBJ("wiz_static_pfxlen").disabled 		= true;
			OBJ("wiz_static_wan_v6addr").value 		= XG(this.rwan3+"/runtime/inf/inet/ipv6/ipaddr");
			OBJ("wiz_static_pfxlen").value 			= XG(this.rwan3+"/runtime/inf/inet/ipv6/prefix");
		}
		else
		{
			OBJ("wiz_static_wan_v6addr").disabled 	= false;
			OBJ("wiz_static_pfxlen").disabled 		= false;
			OBJ("wiz_static_wan_v6addr").value 		= XG(this.rwan4+"/runtime/inf/inet/ipv6/ipaddr");
			OBJ("wiz_static_pfxlen").value 			= XG(this.rwan4+"/runtime/inf/inet/ipv6/prefix");
		}		
	},
	ShowCurrentStage: function()
	{
		...
		{			
			...
			//alert("Jerry_type:"+type);		// Scan all types.
	...
	OnClickNext: function()
	{
		...
		if (stage == "stage_etherv6")
		{
			...
			{
				alert("<?echo I18N("j", "Can't select this mode since IPv4 Internet Connection is 'DS-Lite'.");?>");
				return;
			}
		}
	...
	{		
		...
		// action = "WANDETECTV6" -> "WANTYPERESULT" ( send event("WANV6.DETECT") to trigger WANV6-DETECT.sh ), by Jerry Kao.		                                                            
		// alert("Jerry_WanDetectv6: "+action);		// Run this line.
	...
	WanDetectCallback: function(result, reason)
	{
		...
		{
			case "OK":
				//setTimeout('PAGE.WanDetectv6("WANTYPERESULTV6")', 20000);
				setTimeout('PAGE.WanDetectv6("WANTYPERESULTV6")', 120000);
				break;
			case "PPPoE":
			...
			case "DSLITE":
				for(var i=0; i < this.stages.length; i++)	if(this.stages[i]==="stage_finishv6")	this.currentStage=i;
				for(var i=0; i < this.wanTypes.length; i++)	if(this.wanTypes[i]==="DSLITE")	this.currentWanType=i;
				OBJ("mainform").setAttribute("modified", "true");	
				this.PreWANSettings();
				this.ShowCurrentStage();	
				break;
			case "6RD":
				for(var i=0; i < this.stages.length; i++)	if(this.stages[i]==="stage_finishv6")	this.currentStage=i;
				for(var i=0; i < this.wanTypes.length; i++)	if(this.wanTypes[i]==="6RD")	this.currentWanType=i;
			...
			case "":	// Prolong detecting time (2 -> 5) in Wizard of IPv6, modified by Jerry Kao.
/*			
				if (this.wanDetectNum < 9)
				{
					setTimeout('PAGE.WanDetectv6("WANTYPERESULTV6")', 10000);
					this.wanDetectNum++;
				}
*/				
				if (this.wanDetectNum < 3)
				{
					setTimeout('PAGE.WanDetectv6("WANTYPERESULTV6")', 60000);
					this.wanDetectNum++;
				}
				else
				{
