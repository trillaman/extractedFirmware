...
Page.prototype =
{
	...
	PreSubmit: function()
	{
		...
		if(OBJ("en_ddns").checked)
		{
			...
			{			
				alert("Please select server address.");
				return null;	
			}	
			...
			{			
				alert("Please input the user account.");
				return null;	
			}	
			...
			{
				BODY.ShowAlert("<?echo i18n("Password and Verify Password do not match the new User password.");?>");
				return null;
			}
			if (!TEMP_IsDigit(OBJ("timeout").value))
			{
				BODY.ShowAlert("<?echo i18n("Invalid period. The range of Timeout is 1~8670.");?>");
				OBJ("timeout").focus();
				return null;
			}
	...
	GetReport: function(xml)
	{
		...
		var self = this;
		{
			if (xml.Get("/ddns4/valid")==="1" )
			{
				...
				if (s === "IDLE")
				{
					if		(r === "SUCCESS") msg = "<?echo i18n("Connected");?>";
					else					  msg = "<?echo i18n("Disconnected");?>";
				}
				else
				{
					msg = "<?echo i18n("Disconnected");?>";
				}
			}
			else
			{	
	...
	AddDDNS: function()
	{
		...
		{														
			...
			{
				alert('<?echo i18n("The IPv6 address");?>"'+' '+e.v6addr+'\"<?echo i18n(" is already existed!");?>');
				return null;
			}
			
			if(this.cfg[j].hostname == e.hostname)
			{
				alert('<?echo i18n("The IPv6 host name");?>"'+' '+e.hostname+'\"<?echo i18n(" is already existed!");?>');
				return null;
			}
		}	
		...
		*/
		...
		{			
			alert('<?echo i18n("The maximum number of list is");?>'+' '+max_cnt);								
		}
	}		
};
