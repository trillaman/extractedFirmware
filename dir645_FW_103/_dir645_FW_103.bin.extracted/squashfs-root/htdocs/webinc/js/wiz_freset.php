...
Page.prototype =
{
	...
	phyinf2: null,
	randomkey: null,
	stages: new Array ("stage_desc", "stage_wan_detect","stage_ether", "stage_ether_cfg", "stage_wlan_set", "stage_passwd", "stage_tz", "stage_wlan_result", "check_wan_connect"),
	...
	ShowCurrentStage: function()
	{
		...
		if (this.stages[this.currentStage]=="check_wan_connect")
		{
			...
			setTimeout('PAGE.CheckInternet()', 30000);
		}
	},
	...
	OnClickNext: function()
	{
		...
		{
			if(OBJ("wiz_ssid").value.charAt(0)===" "|| OBJ("wiz_ssid").value.charAt(OBJ("wiz_ssid").length-1)===" ")
			{
	...
	{
		if(confirm("<?echo i18n("Do you want to abandon all changes you made to this wizard?");?>"))
		{
	...
	{
		if(PAGE.internetCheck%4==0)	var action="ping";
		else var action="pingreport";
		...
		ajaxObj.onCallback = function (xml)
		{
			...
			{
				var msgArray = ["<?echo i18n("The internet check is failed. Do you want to restart the wizard again?");?>",
				"<div><span><input type='button' value='<? echo I18N('h', 'Cancel');?>' onclick='javascript:self.location.href=\"\"'></span><span><input type='button' value=\"<? echo I18N('h', 'Restart');?>\" onclick='javascript:self.location.href=\"wiz_freset.php\"'></span></div>"];
				BODY.ShowMessage('<?echo i18n("Internet connection failed");?>', msgArray);
			}		
			else setTimeout('PAGE.CheckInternet()', 1000);
		}
		ajaxObj.setHeader("Content-Type", "application/x-www-form-urlencoded");
