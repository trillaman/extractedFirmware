...
<div class="blackbox" id="bsc_internet">
	...
	<dl>
		...
		<dd><?
			...
			<dl>
				<dt <?if ($FEATURE_NOAPMODE=="1" || query("/runtime/device/modelname")=="DIR-865L") echo ' style="display:none;"';?>><?echo i18n("Access Point Mode");?></dt>
				<dd <?if ($FEATURE_NOAPMODE=="1" || query("/runtime/device/modelname")=="DIR-865L") echo ' style="display:none;"';?>><?
				...
				?></dd>
				<dt <?if ($FEATURE_NOAPMODE=="1" || query("/runtime/device/modelname")!="DIR-865L") echo ' style="display:none;"';?>><?echo i18n("Bridge Mode");?></dt>
				<dd <?if ($FEATURE_NOAPMODE=="1" || query("/runtime/device/modelname")!="DIR-865L") echo ' style="display:none;"';?>><?
...
<div class="blackbox" id="bsc_web_access">
	...
	<p><?echo i18n("Web File Access allows you to use a web browser to remotely access files stored on an SD card or USB storage drive plugged into the router. To use this feature, check the Enable Web File Access checkbox, then create user accounts to manage access to your storage devices or use the Admin or Guest account (Guest/guest) to access the Guest Folder. After plugging in an SD card or USB storage drive, the new device will appear in the list below.");?></p>
	<dl>
		...
		<dl>
			...
			<dd><?
				...
				?>
				<dl>
					<dt><strong><?echo i18n("Admin and Guest be default accounts");?></strong></dt>
					<dd><?
						echo i18n('Admin account will prompt for users to provide the same username and password used to access the Router Web GUI, and it should allow to access root directory for all USB or SD card that device detection. Guest account may setting specific path and shall allow ready only permission, Guest Access Path may be deleted, but the account must exist then Access Path show "None".');
						?>
					</dd>						
				</dl>
				<dl>
					...
					<dd>
						<?
						echo i18n('User shall click User List, "Edit" icon for more detail setting. For Guest Account, Guest user name, password and permission (Read Only, Read/ Write) should not be modified, only may to change device link and folder setting. Device Link, access file folder setting, permission (Read Only, Read/ Write) and Append setting options. Click "Append" button to add more Access Path. Guest Account must support maximum two Access Paths, others account must support maximum five Access Paths.');
						?>
					</dd>
				</dl>
			</dd>
