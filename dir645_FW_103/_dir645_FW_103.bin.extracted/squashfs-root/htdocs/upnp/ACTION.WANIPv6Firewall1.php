...
if($addrtype == "ipv6") 
{ 	
	$G_IGD = "urn:schemas-upnp-org:device:InternetGatewayDevice:2";
}

$dev_nodebase = UPNP_getdevpathbytype($INF_UID, $G_IGD);
if ($dev_nodebase == "")
{
	SHELL_debug($SHELL_FILE, "[WANIPv6Firewall] Can not to get the runtime node path of IGD!");
	exit;
}

$act_name	= query($ACTION_NODEBASE."/action_name");
$svc_type	= query($dev_nodebase."/devdesc/device/deviceList/device:1/deviceList/device:1/serviceList/service:3/serviceType");


TRACE_debug("  === [IPv6Firewall] INF_UID: ". $INF_UID);
TRACE_debug("  ===    G_IGD: ". $G_IGD);
TRACE_debug("  === act_name: ". $act_name);
TRACE_debug("  === svc_type: ". $svc_type);


$WID = 1; /* WAN port ID */
$errorCode = 401;
$SOAP_BODY="";
if (	$act_name == "GetFirewallStatus"	
	||	$act_name == "AddPinhole"
	||	$act_name == "UpdatePinhole"
	||	$act_name == "DeletePinhole"
	||	$act_name == "GetPinholePackets"	
	||	$act_name == "CheckPinholeWorking"
	||	$act_name == "GetOutboundPinholeTimeout")
{
