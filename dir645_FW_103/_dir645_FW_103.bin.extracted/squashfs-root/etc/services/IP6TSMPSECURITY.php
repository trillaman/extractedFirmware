...
function LAN_security($stsp, $name, $dev, $CHAIN, $CHAIN_CommFWD, $debug)
{
...
function WAN_security($stsp, $name, $dev, $CHAIN, $CHAIN_CommFWD, $debug)
{
	/* Special Address Definition  */
	$gl_uni_pfx = "2000::/4";	// Global unicast prefix.		
...
function Comm_FWD_chain($CHAIN_FWD, $debug)
{
	/* Special Address Definition  */
	$gl_uni_pfx  = "2000::/4";			// Global unicast prefix
	...
	startcmd("# Common Chain: FWD.SECURITY");
	...
	/* REC-10 */
	debug_SmpSecurity("REC-10: SHOULD NOT fwd ICMPv6 destination-unreachable and packet-too-big.", $debug);
	...
	/* REC-19 */
	debug_SmpSecurity("REC-19: Recept of and sort of ICMPv6 message MUST NOT terminate the UDP state record.", $debug);
	...
	/* REC-33 */
	debug_SmpSecurity("REC-33: MAY provide filtering behavior options (endpoint- or address-independent) by administrator.", $debug);
...
if ($smp_security == "1")
{
	...
	Comm_FWD_chain($CHAIN_CommFWD, $debug_SmpSecurity);
		...
		{			
			...
			if ($stsp!="")
			{
				...
				{				
					...
					if ($laninf!="")
					{ 
						LAN_security($stsp, $name, $laninf, $CHAIN_pfx.$name, $CHAIN_CommFWD, $debug_SmpSecurity);						
					}
				}
			}
		...
		{			
			...
			if ($stsp!="")
			{
				...
				{					
					...
					if ($waninf!="") 
					{
						WAN_security($stsp, $name, $waninf, $CHAIN_pfx.$name, $CHAIN_CommFWD, $debug_SmpSecurity);						
					}
				}
			}
